...
int run_server(const char *port, bool gso, const char *logfile, const char *cc, int iw, const char *cert, const char *key)
{
    ...
    //Connection level buffer set here:
    server_ctx.transport_params.max_data = 32*1024*1024; //Set to 32 MB, adjust if needed. Default if 16MB, but can go all the way to UINT32_MAX
    
    if(strcmp(cc, "reno") == 0) {
        server_ctx.init_cc = &quicly_cc_reno_init;
    } else if(strcmp(cc, "cubic") == 0) {
        server_ctx.init_cc = &quicly_cc_cubic_init;
    }else if(strcmp(cc, "wrc") == 0) {
    	server_ctx.init_cc = &quicly_cc_wrc_init;
    }
