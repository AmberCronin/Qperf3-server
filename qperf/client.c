...
int run_client(const char *port, bool gso, const char *logfile, const char *cc, int iw, const char *host, int runtime_s, bool ttfb_only)
{
    ...
    client_ctx.transport_params.max_data = 32*1024*1024; //Set to 32 MB, adjust if needed. Default if 16MB, but can go all the way to UINT32_MAX
    ...
    if(strcmp(cc, "reno") == 0) {
       ...
       client_ctx.init_cc = &quicly_cc_wrc_init;
        }
